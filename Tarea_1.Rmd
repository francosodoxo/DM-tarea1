...
sumData = data.frame(localidad=c(),total=c())
for (i in 1:nrow(data_tf)) {
  append(sumData$localidad, data_tf[i,1],after = length(sumData$localidad))
  append(sumData$total, sum(data_tf[i,3:113]), after = length(sumData$total))
}
typeof(sumData$total)
sumData[order(sumData$total, decreasing= TRUE)]
```
